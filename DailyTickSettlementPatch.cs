...
namespace SafeWarLogPatch
{
    ...
    public static class SafeDailyTickSettlementPatch
    {
        /// <summary>
        ...
        static bool Prefix(Settlement settlement)
        {
            ...
            {
                SafeLog.Error($"[SafeDailyTickSettlementPatch] Unexpected error in Prefix: {ex}");
                return false;
            }
        }
        ...
        /// </summary>
        static void Finalizer(Settlement settlement, Exception __exception)
        {
            if (__exception == null)
                return;

            SafeLog.Warn($"[SafeDailyTickSettlementPatch] Suppressed exception in DailyTickSettlement:\n  {__exception}");

            if (settlement == null)
                return;

            SafeLog.Warn($"[SafeDailyTickSettlementPatch] Attempting to repair broken settlement: {settlement.Name?.ToString() ?? "[unknown]"}");
                ...
                foreach (var rebellionsBehavior in allRebelBehaviors)
                {
                        ...
                        if (fieldInfo != null)
                        {
                            ...
                            if (dictionary != null)
                            {
                                if (dictionary.Contains(settlement))
                                {
                                    ...
                                    if (rebelParty != null)
                                    {
                                        SafeLog.Info($"[SafeDailyTickSettlementPatch] Disbanding rebel party '{rebelParty.Name}' created for settlement '{settlement.Name}'");
                                        try
                                        {
                                        ...
                                        {
                                            SafeLog.Warn($"[SafeDailyTickSettlementPatch] Unable to RemoveParty on rebel party: {disEx}");
                                        }
                                    }

                                    // Remove the dictionary entry so the behavior does not try again
                                    dictionary.Remove(settlement);
                                    SafeLog.Info($"[SafeDailyTickSettlementPatch] Removed settlement '{settlement.Name}' from _settlementToRebelParty dictionary.");
                                }
                            }
                        }
                    }
                    ...
                    if (fieldTickInfo != null)
                    {
                        ...
                        if (nextRebellionDict != null && nextRebellionDict.Contains(settlement))
                        {
                            ...
                            SafeLog.Info($"[SafeDailyTickSettlementPatch] Bumped nextRebellionTickFor[{settlement.Name}] to avoid immediate retry.");
                        }
                    }
                    ...
                    if (privateRebelFaction != null)
                    {
                        ...
                        SafeLog.Info($"[SafeDailyTickSettlementPatch] Cleared private _rebelFaction on settlement '{settlement.Name}'.");
                    }
                }
            }
            catch (Exception repairEx)
            {
                SafeLog.Error($"[SafeDailyTickSettlementPatch] Error while repairing settlement '{settlement.Name}': {repairEx}");
            }
        }
    }
}
