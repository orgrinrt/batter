...
using Bannerlord.ButterLib.ObjectSystem.Extensions; // ← for SetFlag / HasFlag
...
namespace SafeWarLogPatch
{
    public class HeroClanSanitizerBehavior : CampaignBehaviorBase
    {
        ...
        {
            SafeLog.Info("[Sanitizer] Running hero data sanitization...");
            ...
            {
                if (hero == null || ShouldSkip(hero))
                {
                    skippedCount++;
                    continue;
                }

                if (_alreadySanitized.Contains(hero.StringId))
                {
                    skippedCount++;
                    continue;
                }
                ...
                try
                {
                    if (SanitizeHero(hero))
                    {
                        _alreadySanitized.Add(hero.StringId);
                        MarkHeroAsSanitized(hero);
                        sanitizedCount++;
                    }
                    else
                    {
                ...
                {
                    SafeLog.Error($"[Sanitizer] Error processing hero {SafeHeroName(hero)}: {ex}");
                }
            }

            SafeLog.Info($"[Sanitizer] Completed: Sanitized {sanitizedCount} heroes, skipped {skippedCount} heroes.");
        }

        private bool SanitizeHero(Hero hero)
        {
            ...
            {
                var inferredCulture = InferCulture(hero);
                if (inferredCulture != null && !Equals(hero.Culture, inferredCulture))
                {
                    hero.Culture = inferredCulture;
                    SafeLog.Info($"[Sanitizer] Assigned culture '{inferredCulture.Name}' to hero {SafeHeroName(hero)}");
                    changed = true;
                }
            }

            // 2) Generate parents if missing
            changed |= GenerateParentsForHero(hero);

            // 3) Assign name if missing/placeholder
            changed |= AssignHeroNameIfMissing(hero);

            // 4) Fix clan & faction
            changed |= FixClanAndFaction(hero);

            // 5) Ensure at least one friend
            changed |= AssignFriendIfNone(hero);
        ...
        private CultureObject InferCulture(Hero hero)
        {
            ...
            foreach (var other in Hero.AllAliveHeroes)
            {
                ...
                int rel = CharacterRelationManager.GetHeroRelation(hero, other);
            ...
            if (allCultures != null && allCultures.Count > 0)
                return allCultures[_random.Next(allCultures.Count)];
        ...
        private bool AssignHeroNameIfMissing(Hero hero)
        {
            if (hero.Name != null && !IsPlaceholderName(hero.Name.ToString()))
                return false;
            ...
            if (culture == null)
            {
                SafeLog.Warning($"[Sanitizer] Cannot assign name: no culture for {SafeHeroName(hero)}");
                return false;
            }
            ...
            NameGenerator.Current.GenerateHeroNameAndHeroFullName(
                ...
                useDeterministicValues: false // or true if you want deterministic
            );
            ...
            if (fullName != null)
            {
                ...
                SafeLog.Info($"[Sanitizer] Assigned generated name '{fullName}' to hero {SafeHeroName(hero)}");
                return true;
            }
            else
            {
                SafeLog.Warning($"[Sanitizer] NameGenerator produced no name for {SafeHeroName(hero)}");
                return false;
            }
        }
        ...
        private bool GenerateParentsForHero(Hero hero)
        {
            bool changed = false;
            ...
            if (hero.Father == null)
            {
                Hero newDad = CreateNewRandomHero(culture);
                if (newDad != null)
                {
                    ...
                    hero.Father = newDad;
                    SafeLog.Info($"[Sanitizer] Created father '{SafeHeroName(newDad)}' for {SafeHeroName(hero)}");
                    changed = true;
                }
            }

            Hero newMom = CreateNewRandomHero(culture);
            if (newMom != null)
            {
                newMom.ChangeState(Hero.CharacterStates.Dead);

                hero.Mother = newMom;
                SafeLog.Info($"[Sanitizer] Created mother '{SafeHeroName(newMom)}' for {SafeHeroName(hero)}");
                changed = true;
            }
        ...
        private bool AssignFriendIfNone(Hero hero)
        {
            ...
            Hero friend = CreateNewRandomHero(culture);
            if (friend == null) return false;

            // If they already share a relation, skip
            if (AreAlreadyFriends(hero, friend)) return false;

            // Ensure friend has a valid clan/faction
            FixClanAndFaction(friend);
            ...
            SafeLog.Info($"[Sanitizer] Assigned new friend '{SafeHeroName(friend)}' to hero {SafeHeroName(hero)}");
            return true;
        }
        ...
        public static bool FixClanAndFaction(Hero hero)
        {
            if (hero.IsWanderer)
                return false;
            ...
            Clan clan = hero.Clan ?? hero.Father?.Clan ?? hero.Mother?.Clan;
            // 2) Attempt to infer faction from hero or parents
            IFaction faction = hero.MapFaction ?? hero.Father?.MapFaction ?? hero.Mother?.MapFaction;

            // 3) If no clan, create a fallback one
            if (clan == null)
            {
                clan = CreateFallbackClanForHero(hero);
                SafeLog.Info($"[Sanitizer] Created fallback clan for {SafeHeroName(hero)}");
                changed = true;
            }
            ...
            if (hero.MapFaction == null)
            {
                ...
                {
                    kingdomToUse = Kingdom.All[_random.Next(Kingdom.All.Count)];
                }
        ...
        public static Clan CreateFallbackClanForHero(Hero hero)
        {
            ...
            string clanStringId = $"sanitized_clan_{hero.StringId}_{_random.Next(10000)}";
            if (Clan.All.Any(c => c.StringId == clanStringId))
            {
                clanStringId = $"{clanStringId}_{DateTime.UtcNow.Ticks}";
            }

            // Create the clan object (using the static factory)
            Clan newClan = Clan.CreateClan(clanStringId);

            // Generate a clan name via NameGenerator:
            // Pass in the hero’s culture and (optionally) the settlement where the clan originates
            Settlement originSettlement = hero.BornSettlement ?? Settlement.All.FirstOrDefault(s => s.Culture == culture);
            ...
            if (generatedName == null)
            {
                ...
                generatedName = new TextObject($"Clan_{hero.FirstName}");
            }
            ...
            Kingdom chosenKingdom = Kingdom.All.FirstOrDefault(k => k.Culture == culture)
            ?? Hero.MainHero?.Clan?.Kingdom
            ?? Kingdom.All.FirstOrDefault();

            if (chosenKingdom != null)
            {
                newClan.Kingdom = chosenKingdom;
            }
        ...
        private Hero CreateNewRandomHero(CultureObject culture)
        {
            ...
            CharacterObject template = candidates[_random.Next(candidates.Count)];

            // 2) Pick a settlement of that culture (or a random one if none match)
            Settlement bornSettlement = Settlement.All
            .FirstOrDefault(s => s.Culture == culture)
            ?? Settlement.All.GetRandomElement();

            // 3) Age (20±5)
            int ageYears = 20 + _random.Next(-5, 6);

            // 4) Create it
            Hero newHero = HeroCreator.CreateSpecialHero(template, bornSettlement, null, null, ageYears);
            if (newHero == null)
                return null;
            ...
            if (newHero.Name == null)
            {
                ...
                NameGenerator.Current.GenerateHeroNameAndHeroFullName(newHero, out firstName, out fullName, true);
        ...
        private static bool AreAlreadyFriends(Hero a, Hero b)
        {
        ...
        private static bool ShouldSkip(Hero hero)
        {
            ...
            if (hero.HasFlag(SanitizedFlagKey))
        ...
        {
            if (hero == null) return;
            hero.SetFlag(SanitizedFlagKey); // <- use SetFlag(...) instead of SetCustomProperty
        }
